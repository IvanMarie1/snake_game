...
# mettre le jeu sous forme de classe

class Snake:
    UNIT = 40
    dico = {"N": (0, -1), "S": (0, 1), "W": (-1, 0), "E": (1, 0), "": (0, 0)}

    def __init__(self, length: int, pos: list[int, int]):
        self.length: int = length
        self.directions: list[str] = ['E'] * length
        self.positions: list[pg.Vector2] = [pg.Vector2((pos[0]-i)*self.UNIT, pos[1]) for i in range(length)]
    ...
    def change_direction(self, direction: str):
        for i in range(self.length-1):
            self.directions[self.length-i] = self.directions[self.length-i-1]
        self.directions[0] = direction
    
    def move_snake(self, distance: float):
